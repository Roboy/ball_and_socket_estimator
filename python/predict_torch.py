...
def magentic_data_callback(data):
    ...
    x_test = sensors_scaler[data.id].transform(x_test).astype('float32').reshape(-1, 4, 3)
    ...
    if use_lstm[data.id]:
        ...
        if not model[data.id].init_prediction_belief:
            x_in = torch.tensor(history[data.id][None, :, :], dtype=torch.float32)
            out_set = model[data.id].predict_init(x_in, n_initial_obs=history_n)
        else:
            x_in = torch.tensor(x_test, dtype=torch.float32)
            out_set = model[data.id].predict_one_step(x_in)

        out_set = compute_rotation_matrix_from_ortho6d(out_set)
        output = compute_euler_angles_from_rotation_matrices(out_set).detach().numpy()
...
if __name__ == '__main__':
    ...
    for i in range(len(model)):
        ...
        if body_part not in ["shoulder_right", "shoulder_left"]:
            continue

        model_path = f'{base_path}/outputs/{body_part}_dvbf_ad_rot6D'
        # model_path += "_lstm" if us6e_lstm[i] else "_tanh"
        ...
        if os.path.isdir(model_path):
            ...
            model[i] = DVBF.load_from_checkpoint(checkpoint_path=f'{model_path}/best.ckpt')
