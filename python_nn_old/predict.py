...
from roboy_control_msgs.msg import JointTrackingLoss
...
use_lstm = [True, False, True, True, True]
...
jointTrackingLossPublisher = rospy.Publisher("/roboy/pinky/control/JointTrackingLoss", JointTrackingLoss)
...
def magentic_data_callback(data):
    ...
    tracking_msg = JointTrackingLoss()
    tracking_msg.header = std_msgs.msg.Header()
    tracking_msg.header.stamp = rospy.Time.now()
    tracking_msg.name = BodyPart[MagneticId(data.id).name]

    # if BodyPart[MagneticId(data.id).name] == "shoulder_right":
    #     rospy.loginfo(str(BodyPart[MagneticId(data.id).name]) + "\t" + str(error))

    if error < 0.1:
        tracking_msg.tracking = True
    else:
        tracking_msg.tracking = False
    ...
    jointTrackingLossPublisher.publish(tracking_msg)
