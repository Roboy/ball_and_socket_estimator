...
class Space:
    def __init__(self, n_particles, body_part):
        ...
        self.minima[2] = -0.7 # -0.6
        self.maxima[2] = 0.7 # 0.6
        self.minima[3] = 0.0
        self.maxima[3] = 0.73
    ...
    def set_gbest(self):
        ...
        for t in lin_target:
            ...
            joint_targets_msg.header = self.joint_targets_msg.header
            joint_targets_msg.name = [self.joint_targets_msg.name[i] for i in pub_idx]
            joint_targets_msg.position = [self.joint_targets_msg.position[i] for i in pub_idx]
            joint_targets_msg.velocity = [self.joint_targets_msg.velocity[i] for i in pub_idx]
            joint_targets_msg.effort = [self.joint_targets_msg.effort[i] for i in pub_idx]

            self.joint_targets_pub.publish(joint_targets_msg)
            self.rate.sleep()

        # sleep_s = np.random.randint(0, 3)
        # rospy.sleep(sleep_s)
